...
export const fetchActivitiesList = (bookingId, offset, refresh, apiOptions = {}) => (dispatch, getState) => {
  ...
  if (isPublic) {
    if (isAll) {
      apiUrl = !isLoggedIn ? `${Api.getRecentActivity}${bookingId}/public_list/${isBookingId ? `?starsona=true` : ''}` : `${Api.getRecentActivity}${bookingId}/${isBookingId ? `?starsona=true` : ''}`;
    } else {
      apiUrl = !isLoggedIn ? `${Api.getRecentActivity}${bookingId}/public_list/?offset=${offset}&limit=${limit}${isBookingId ? `&starsona=true` : ''}` : `${Api.getRecentActivity}${bookingId}/?offset=${offset}&limit=${limit}${isBookingId ? `&starsona=true` : ''}` ;
    }
  } else if (isAll) {
    apiUrl = `${Api.getRecentActivity}${bookingId}/${isBookingId ? `?starsona=true` : ''}`;
  } else {
    apiUrl = `${Api.getRecentActivity}${bookingId}/?offset=${offset}&limit=${limit}${isBookingId ? `&starsona=true` : ''}`;
  }
