...
export const getRequestDetails = (bookingId, isLoggedIn, isBookingId) => {
  let apiUrl = `${Api.getRequestDetails}${bookingId}/${isBookingId ? '?starsona=true' : ''}`;
  if (!isLoggedIn) {
    apiUrl = `${apiUrl}get/${isBookingId ? '?starsona=true' : ''}`;
  }
  return fetch(apiUrl)
    .then(resp => resp.data);
};
