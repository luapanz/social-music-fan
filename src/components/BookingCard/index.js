...
const BookingCard = (props) => {
  ...
  useEffect(() => {
    if (props.bookingModal.requestId) {
      getRequestDetails(props.bookingModal.requestId, props.isLoggedIn, true)
        .then((requestDetails) => {
  ...
  const onFanCompleteAction = (type, data) => {
    ...
    if (type === 'tip') {
      ...
      props.fetchActivitiesList(newRequestData.booking_id, 0, true, {isBookingId: true});
...
const mapDispatchToProps = dispatch => ({
  ...
  fetchActivitiesList: (bookingId, offset, refresh, apiOptions) => dispatch(fetchActivitiesList(bookingId, offset, refresh, apiOptions)),
