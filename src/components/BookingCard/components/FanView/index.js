...
const FanView = (props) => {
  ...
  useEffect(() => {
    ...
    if (isMobile) {
      props.fetchActivitiesList(bookingData.booking_id, 0, true, {isBookingId: true, isPublic: props.modalData.isPublic, isAll: true});
    } else {
      props.fetchActivitiesList(bookingData.booking_id, 0, true, {isBookingId: true, isPublic: props.modalData.isPublic});
    }
  ...
  const fetchActivity = (offset, refresh) => {
    props.fetchActivitiesList(props.bookingData.id, offset, refresh, {isBookingId: true, isPublic: props.modalData.isPublic});
  }
