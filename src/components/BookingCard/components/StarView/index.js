...
const StarView = (props) => {
  ...
  useEffect(() => {
    ...
    if (isMobile) {
      props.fetchActivitiesList(bookingData.booking_id, 0, true, {isAll: true, isBookingId: true});
    } else {
      props.fetchActivitiesList(bookingData.booking_id, 0, true, {isBookingId: true});
    }
  ...
  const fetchActivity = (offset, refresh) => {
    props.fetchActivitiesList(props.bookingData.id, offset, refresh, {isBookingId: true});
  }
